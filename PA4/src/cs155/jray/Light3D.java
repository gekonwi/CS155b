...
/**
  This represents 3D lights with the full OpenGL color model.
  So there are several parameters including Color3D values for
  the ambient, diffuse, and specular illumination as well as the attenuation
  coefficients: attC, attL, attQ - representing the Constant, Linear, and Quadratic components
  Finally we include an intensity to make it easy to increase the brightness of a all components of a light.
 **/
...
public class Light3D {
  ...
  public Light3D( Point3D new_location, double intensity, Color3D amb, Color3D dif, Color3D sp  ){
	location= new Point3D(new_location);
        this.intensity = intensity;
        this.ambient=amb; this.diffuse = dif; this.specular=sp;
  }
    ...
    /** calculate the attentuation of the light based on its distance from the illuminated point **/
    ...
    **/
    ...
    public static double specular(Point3D lightVec, Point3D normal, Point3D eyeVec, int hardness) {
