...
public class RayTracer3D {
	...
	public static Color3D computeColor(Ray3D r, Scene3D s) {
		List<RayHit> closestHits = s.objs.rayIntersects(r);

		if (closestHits.isEmpty())
			return s.backgroundColor;

		if (closestHits.size() == 1)
			return computeColor(r, s, closestHits.get(0));

		if (closestHits.get(0).hashCode() < closestHits.get(1).hashCode())
			return computeColor(r, s, closestHits.get(0));
		else
			return computeColor(r, s, closestHits.get(1));
	}

	private static Color3D computeColor(Ray3D r, Scene3D s, RayHit hit) {
		...
		// pixelcolor
		for (Light3D light : s.lights) {
			if (isObscured(light, hit.hitPoint, s.objs))
				continue;
	 ...
	 * @return
	 */
	private static boolean isObscured(Light3D light, Point3D point, Group3D objs) {
		...
		List<RayHit> closesHits = objs.rayIntersects(r);
		if (closesHits.isEmpty())
			return false;

		return isObscurred(point, distToLight, closesHits.get(0));
	}

	private static boolean isObscurred(Point3D point, double distToLight,
			RayHit hit) {
		if (hit == RayHit.NO_HIT)
			return false;
