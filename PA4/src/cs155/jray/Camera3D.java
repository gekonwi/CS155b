 ...
 * z-axis (i.e. in the direction (0,0,-1))  The camera object is responsible for generating
 * rays corresponding to integer-valued screen coordinates. The screen coordinates
 * depend on the size of the screen which is specified by the Film object.
 * 
 * You create a camera by passing in a film parameter which determines how the
 * pixels will be converted to rays based on the width and height of the film object
...
public class Camera3D {
	...
	public Ray3D generateRay(int i, int j) {
		/*
		 * the screen coordinates have the y axis pointed down!
		 * so we need to compensate by letting k represent the distance in pixels
		 * from the base!
		 */
		int k = film.height()-j; 
