...
//	
//	float trPrism[][] = {
//			v1, v2, v3, 	// triangle bottom
//			v4, v6, v5		// triangle top
//			// ...
//	};
//	
...
//}
	...
	private float[] vertices() {
		...
		return redundantPoints(vertices, indices);
	}
	
	// list a new point for each entry in indices
	// take the values from points according to the respective index
	// from indices (this redundancy seems to be needed by OpenGL ...)
	private float[] redundantPoints(float[] points, byte[] indices) {
	...
	// since vertices() creates the (redundant) vertices in the order of
	// the faces the indices are just an iteration over the vertices
	private byte[] indices(float[] vertices) {
		byte[] indices = new byte[vertices.length / 3];
	...
	private float[] normals() {
		...
		return redundantPoints(normals, indices);
	}
	...
	private byte[] indices = indices(vertices);
