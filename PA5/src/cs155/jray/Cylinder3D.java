...
public class Cylinder3D extends Object3D
{
    ...
    public Cylinder3D (Point3D center, Point3D direction, double radius, double height) {
	this(center,direction,radius,height, Material.defaultMat);
    }

    public Cylinder3D (Point3D center, Point3D direction, double radius, double height, Material m)
     {
            this.center = center;
     ...
     public RayHit rayIntersect(Ray3D r) {
      ...
      double 
	   ...
	   h = direction.dot(V2);
	   if ((h>= 0) && (h <= height)) 
		   return new RayHit(p,t1,normal(p),this,getTextureCoordinate(V2));
       }
       if (t2 >= epsilon) {
           p = r.atTime(t2);
	   V2 = p.subtract(center);
	   ...
	   if ((h>= 0) && (h <= height)) 
		   return new RayHit(p,t2,normal(p),this,getTextureCoordinate(V2));
       }
     ...
     /**
      * calculate the texture coordinate for the point p on the cylinder.
      * the y texture coordinate should be the height above the base
      * the x texture coordinate should be the radians when you project
      * the x and z coordinates of p into the y=0 plane.  This won't work if 
      * the normal is parallel to the y=0 plane!! 
      * @param p
      * @return
      */
