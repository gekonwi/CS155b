...
public class RayTracer3D {
	...
	static final double RAY_START_OFFSET = 0.01;
	...
	public static Color3D computeColor(Ray3D r, Scene3D s, int depth) {
		...
		if ((m.reflect > 0) && (depth > 0)) {
			...
			Ray3D reflRay = calcReflectionRay(r, hit);
			Color3D reflectColor = computeColor(reflRay, s, depth - 1);
	 ...
	 * @return
	 */
	static Ray3D calcReflectionRay(Ray3D r, RayHit hit) {

		// project the incoming ray direction on the normal of the hit shape
		Point3D dProj = hit.normal.scale(r.d.dot(hit.normal));

		// define the direction of the reflected ray
		Point3D reflRayDir = r.d.subtract(dProj.scale(2));

		Point3D reflRayStart = hit.hitPoint.add(reflRayDir.scale(RAY_START_OFFSET));

		// return the reflected ray
		return new Ray3D(reflRayStart, reflRayDir);
	}
	...
	public static void drawScene(Scene3D s) {
		...
		for (int i = 0; i < w; i++) {
			...
			for (int j = 0; j < h; j++) {
				...
				Color3D pixelColor = computeColor(r1, s, s.depth); // compute
																	// its color
